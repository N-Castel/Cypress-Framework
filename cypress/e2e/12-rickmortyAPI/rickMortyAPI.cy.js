...
describe('API Testing', () => {
    ...
    rickMortyFixture.forEach(rickMortyFixture => {
        ...
        it('API - delete existing ToDo', () => {
            ...
            cy.getAPI(rickMortyFixture.endpointAddRickMorty)
            .its('body')
            .should('have.lenght', 826)
        })

        it.only('Mocked API response  - ', () => {
            cy.intercept('GET', 'https://rickandmortyapi.com/api/character/10', interceptRickMorty.characterIntercept).as('interceptAPI')
            cy.request('https://rickandmortyapi.com/api/character/10')
            cy.log('test')
